...
// Global declaration handled in types/global.d.ts or similar
...
export function PullToRefresh({
  ...
  const handleGesture = useCallback(
    (gesture: TouchGesture) => {
      if (gesture.type === 'pull-to-refresh' && !isRefreshing) {
        ...
        // Handle async refresh without returning promise to void callback
  ...
  return (
    <div className="relative">
      {/* Pull indicator */}
      <div
        className="absolute left-0 right-0 top-0 z-50 flex items-center justify-center border-b border-gray-200 bg-white transition-transform duration-300 ease-out"
        style={{
      ...
      {/* Content */}
      <div className={isPulling ? 'transition-transform duration-300' : ''}>{children}</div>
    </div>
  )
}
...
export function SwipeableCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-lg">
      {/* Left action */}
      <div className="absolute bottom-0 left-0 top-0 z-10 flex w-20 items-center justify-center bg-red-500 text-white">
        {leftAction}
      </div>

      {/* Right action */}
      <div className="absolute bottom-0 right-0 top-0 z-10 flex w-20 items-center justify-center bg-green-500 text-white">
        {rightAction}
      </div>

      {/* Card content */}
      <div
        className="cursor-grab bg-white shadow-md transition-transform duration-300"
        style={{ transform: `translateX(${translateX}%)` }} // eslint-disable-line
      >
        {children}
      </div>
    </div>
...
export function TapArea({
  ...
  return (
    <div
      className={` ${sizeClasses[size]} ${className} ${isActive ? 'scale-95' : ''} cursor-pointer select-none transition-transform duration-150 active:scale-95`}
