...
test.describe('Main Navigation', () => {
  ...
  test('should show logo and link to homepage', async ({ page }) => {
    const logo = page.locator('[data-testid="logo"], nav img[alt*="logo"], a:has-text("PG Closets")').first();
...
test.describe('Search Navigation', () => {
  test('should open search overlay', async ({ page }) => {
    ...
    if (await searchButton.count() > 0) {
      ...
      const searchOverlay = page.locator('[role="dialog"]:has(input[type="search"]), [data-testid="search-overlay"]');
...
test.describe('Navigation Keyboard Accessibility', () => {
  ...
  test('should support Enter key to activate links', async ({ page }) => {
    ...
    // URL should change unless we hit logo which goes to home
  });
