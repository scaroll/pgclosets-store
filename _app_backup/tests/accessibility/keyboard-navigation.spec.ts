...
test.describe('Keyboard Navigation', () => {
  ...
  test.describe('Modal and Overlay Keyboard Interaction', () => {
    test('should close modal with Escape key', async ({ page }) => {
      ...
      const modalTrigger = page
        .locator('button:has-text("Quote"), button[data-modal-trigger]')
        .first()
  ...
  test.describe('Navigation Menu Keyboard Interaction', () => {
    ...
    test('should open dropdown menu with Enter/Space', async ({ page }) => {
      ...
      const dropdownTrigger = page
        .locator('button[aria-expanded], button[aria-haspopup="true"]')
        .first()
  ...
  test.describe('Product Interaction Keyboard Support', () => {
    ...
    test('should open product with Enter key', async ({ page }) => {
      ...
      const firstProduct = page
        .locator('[data-testid="product-card"] a, [data-testid="product-card"] button')
        .first()
  ...
  test.describe('Keyboard Shortcuts', () => {
    test('should have documented keyboard shortcuts', async ({ page }) => {
      ...
      const shortcutsButton = page.locator(
        'button[aria-label*="keyboard"], button:has-text("Shortcuts"), [data-testid="keyboard-shortcuts"]'
      )

      if ((await shortcutsButton.count()) > 0) {
        await shortcutsButton.click()

        const shortcutsPanel = page.locator(
          '[role="dialog"]:has-text("Shortcuts"), [data-testid="shortcuts-panel"]'
        )
  ...
  test.describe('Focus Visibility', () => {
    ...
    test('should not remove focus outline on click', async ({ page }) => {
      ...
      // Note: This test may fail if :focus-visible is used (which is acceptable)
      // The important thing is that keyboard focus is always visible
      // We just verify the button is focused
