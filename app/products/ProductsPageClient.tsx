...
export default function ProductsPageClient() {
  ...
  const handleFilterChange = (newFilters: { category: string; search: string }) => {
    ...
    if (newFilters.category !== "all") {
      filtered = filtered.filter((product) => product.category?.toLowerCase() === newFilters.category.toLowerCase())
    }

    if (newFilters.search) {
      filtered = filtered.filter(
        (product) =>
          (product.name && product.name.toLowerCase().includes(newFilters.search.toLowerCase())) ||
          (product.description && product.description.toLowerCase().includes(newFilters.search.toLowerCase())),
      )
    }
