...
function calculateVolumeDiscount(quantity: number, markedUpPrice: number): number {
  ...
  const discount = config.volumeDiscounts.find(
    d => quantity >= d.minQuantity && (!d.maxQuantity || quantity <= d.maxQuantity)
  )
...
function calculateInstallationCost(
  ...
  const config = OTTAWA_PRICING_CONFIG
  const rate =
    config.installationRates.find(r => r.doorType === doorType) ||
...
function getDeliveryZone(postalCode: string) {
  ...
  return config.deliveryZones.find(zone => zone.postalCodes.some(code => prefix.startsWith(code)))
}
...
function calculateFinancingPayment(amount: number, term: FinancingTerm): FinancingTerm {
  ...
  if (term.interestRate === 0) {
    ...
    monthlyPayment =
      (principal * (monthlyRate * Math.pow(1 + monthlyRate, term.months))) /
...
async function handleReninQuoteRequest(
  ...
  const { error: supabaseError } = await supabase.from('renin_quote_requests').insert(record)
...
function calculateEstimatedDelivery(timeline?: string): string {
  ...
  const deliveryDate = new Date(
    today.getTime() + (baseDelivery + additionalDays) * 24 * 60 * 60 * 1000
  )
...
export const POST = createProtectedRoute(reninQuoteRequestSchema as any, handleReninQuoteRequest, {
