    ...
    const status = {
      ...
      services: {
        ...
        database: {
          ...
          error: dbError && 'message' in dbError ? (dbError as { message: string }).message : null,
          connection: !dbError,
        },
        environment: {
      ...
      performance: {
        ...
        cpu: {
          load_average: process.platform !== 'win32' ? os.loadavg() : [0, 0, 0],
        },
      },
    }

    // Calculate response time
    status.services.api.responseTime = Date.now() - status.services.api.responseTime
