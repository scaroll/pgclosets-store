/**
 * Transform Renin Extracted Products to PG Closets Schema - v2 (Fixed)
 *
...
interface ExtractedReninProduct {
  name: string;
  sku: string;
  price: string | number | { min: number; max: number }; // e.g., "$565.00 – $799.00" or "$565.00" or 509.00
  description: string;
  specifications: {
    sizes?: string[] | string;
 ...
 * Handles ranges like "$565.00 – $799.00", single prices like "$565.00", and numbers like 509.00
 */
...
async function transformAllProducts() {
  console.log('🚀 Starting Renin Product Transformation v2...\n');
